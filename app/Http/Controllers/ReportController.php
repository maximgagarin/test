...
class ReportController extends Controller
{
   ...
   public function calc()
   {
       ...
       $PaidChvznos = Payment::leftJoin('payment_movs', 'payments.id', '=', 'payment_movs.payments_id')->where('type', 'чвзнос')->whereNull('prepays')->whereBetween('payment_movs.date', [$date1, $date2])->sum('payment_movs.sum');
       ...
       $PaidTrash = Payment::leftJoin('payment_movs', 'payments.id', '=', 'payment_movs.payments_id')->where('type', 'мусор')->whereNull('prepays')->whereBetween('payment_movs.date', [$date1, $date2])->sum('payment_movs.sum');
       ...
       $PaidRoad = Payment::leftJoin('payment_movs', 'payments.id', '=', 'payment_movs.payments_id')->where('type', 'дороги')->whereNull('prepays')->whereBetween('payment_movs.date', [$date1, $date2])->sum('payment_movs.sum');
       ...
       $PaidBlag = Payment::leftJoin('payment_movs', 'payments.id', '=', 'payment_movs.payments_id')->where('type', 'благоустройство')->whereNull('prepays')->whereBetween('payment_movs.date', [$date1, $date2])->sum('payment_movs.sum');
